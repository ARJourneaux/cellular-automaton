...
func change_shader(shader_num : int):
	var shader_text = FileAccess.open("user://Shaders/shader" + str(shader_num) + ".txt", FileAccess.READ)
	shader_file.source_compute = shader_text.get_as_text()
	var shader_spirv := rd.shader_compile_spirv_from_source(shader_file)
	shader = rd.shader_create_from_spirv(shader_spirv)
	pipeline = rd.compute_pipeline_create(shader)
...
func setup() -> void:
	var initial_image := texture.get_image()
	initial_image.convert(image_format)
	image_size = initial_image.get_size()
...
func _process(_delta: float) -> void:
	# Timing out with delta allows slowing down / speeding up the process function with Engine.time_scale
	time_out -= _delta
	if (time_out < 0):
		compute()
		time_out = 0.01666666666667
